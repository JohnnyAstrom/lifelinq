...
# V0.5 (Historical Milestone)

This document describes architectural decisions and implementation state during the V0.5 milestone.

It is retained for historical reference and context.

It is NOT the current source of truth for feature behavior.
...
- `GET /documents` without `q` returns the full group-scoped list.
- Group scoping remains server-derived and enforced.
...
- **Invariants impacted**
  §1–§3 (group‑scoped + server‑derived), §11 (docs update), §13 (ApplicationService boundary).
...
- A minimal Recipe + Ingredient model exists in Meals with group scoping.
...
- `DELETE /documents/{id}` removes a document when it belongs to the current group.
- Returns `204` on successful delete.
- Returns `404` when the document id does not exist in the current group.
...
- Save meal creates or updates a group recipe via `POST/PUT /meals/recipes`.
...
- Users can reorder open shopping items with press-and-drag in list detail.
- Open-item reorder calls `PATCH /shopping-lists/{listId}/items/{itemId}/order` with `direction` (`UP`/`DOWN`).
