...
Current frontend capture direction: ingredient entry is a lightweight structured row editor with `name`, optional `quantity`, optional `unit`, and implicit `position` from row order. The main `Plan a meal` sheet keeps day + meal type as planning context, while recipe name + ingredients are presented as recipe-backed content inside the same editor. The same editor may now also load an existing saved recipe into the current meal slot via a lightweight picker, but Meals still does not expose a standalone recipe library surface. If a picked existing recipe is left unchanged, the meal keeps using that recipe as-is. If the user changes the picked recipe's name or ingredients inside this slot editor, Meals saves those changes as a new recipe for that meal instead of overwriting the original recipe. Ingredients remain a light summary in the main sheet and open a secondary ingredient sheet for full editing. Shopping intake is now triggered from a separate explicit review/confirm sheet rather than as a passive save-time toggle. That review step may selectively include only some ingredient positions; Meals still saves the full recipe, but only the chosen ingredient occurrences are pushed to Shopping.
