...
Program 5C2 visible-step note: the first real in-app rich-source path now appears as `Import file / PDF` in the same `Add recipe` chooser. That path stays intentionally small: the user chooses one recipe document through the system file picker, LifeLinq routes the selected file into the existing document asset-intake family, and the result continues into the same recipe-draft review flow when downstream extraction can proceed. It is therefore a document-capture path rather than a file-management UI, and it still keeps room for a later separate `Scan photo` path instead of collapsing all asset-backed sources into one visible option.
