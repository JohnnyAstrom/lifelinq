...
In V0.5c, recipes serve primarily as a shopping-generator: ingredients listed on a recipe are used to push shopping items. Recipes now also carry a first recipe-content foundation with `source`, `short note`, and `instructions`, while broader culinary modeling such as nutrition, rich media, and advanced organization remains out of scope.
...
- `Recipe`
  - `id`, `groupId`, `name`, `source`, `shortNote`, `instructions`, `createdAt`, `ingredients`
- `Ingredient`
...
Request body: `name`, `source` (optional), `shortNote` (optional), `instructions` (optional), `ingredients[]` (`name`, `quantity`, `unit`, `position`).  
Response: `recipeId`, `groupId`, `name`, `source`, `shortNote`, `instructions`, `createdAt`, `ingredients`.  
Status: 200 OK.  
Errors: 400 invalid input, 401 missing context, 403 not a group member.
...
Response: `Recipe` response with `source`, `shortNote`, `instructions`, and ingredients.  
Status: 200 OK.  
...
Request body: `name`, `source` (optional), `shortNote` (optional), `instructions` (optional), `ingredients[]` (`name`, `quantity`, `unit`, `position`).  
Response: updated `Recipe` response including `source`, `shortNote`, `instructions`, and ingredients.  
Status: 200 OK.  
