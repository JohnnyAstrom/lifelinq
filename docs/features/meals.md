...
Current image-import stabilization note: after the first OCR-backed image lane and Swedish OCR support, the next bounded quality direction remains intentionally conservative. Image-backed cleanup should prefer rejecting weak ingredient candidates over inventing structure, with special care to keep servings/time/count metadata, instruction headings, short OCR junk, and obvious sponsor/product caption lines out of ingredient rows before review. The next bounded handoff step may preserve short top-title continuations and explicit `Ingredienser` / `Gör så här` section boundaries more calmly before document-style parsing, but broader block reconstruction or layout heuristics remain intentionally out of scope unless smaller line-based shaping steps prove insufficient.
