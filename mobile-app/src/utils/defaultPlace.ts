export function defaultGroupIdFor(userId: string): string {
  const input = utf8Bytes(`personal-group:${userId}`);
...
function md5(bytes: number[]): number[] {
  ...
  for (let i = 0; i < padded.length; i += 64) {
    ...
    for (let j = 0; j < 16; j += 1) {
      const k = i + j * 4;
      block[j] = padded[k] | (padded[k + 1] << 8) | (padded[k + 2] << 16) | (padded[k + 3] << 24);
    }
